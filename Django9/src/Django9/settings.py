...
import os
#reverse_lazy : 기능은 reverse 와 동일 . 웹서버가 완전히 가동됬을 때 url 매칭을 해줌
from django.urls.base import reverse_lazy

#from django.urls.base import reverse
#LOGIN_URL : login_required 데코레이터 / LoginRequiredMixin 클래스를 통해 보여질 로그인 페이지 주소를 작성
LOGIN_URL = reverse_lazy('auth:signin')#reverse('auth:signin')

#LOGIN_REDIRECT_URL : 로그인 후 이동할 페이지
#소셜 로그인 완료 후 보여질 페이지 주소
LOGIN_REDIRECT_URL = reverse_lazy('index')
#LOGOUT_REDIRECT_URL : 로그아웃 후 이동할 페이지

# Build paths inside the project like this: os.path.join(BASE_DIR, ...)
#프로젝트가 현재 위치한 경로를 의미함 src 폴더까지 
...
#구글에서 발급받은 클라이언트 ID
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY='626484878323-aiitka7k2ivsg3kattc9b1p4vt2imcr6.apps.googleusercontent.com'
#구글에서 발급받은 보안 비밀
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET='iSrl7DWO1ohSPekrv55xvjXA'
...
AUTHENTICATION_BACKENDS=(
    ...
    #소셜 로그인 정보를 Django의 User 모델클래스에 매칭(장고 DB에 저장)
...
#아무것도 적지않은 경우 '127.0.0.1'로만 요청가능
...
INSTALLED_APPS = [
    ...
    'blog',
    #django-disqus 모듈 설치해야함
    'disqus', #댓글 사이트와 연동하기 위한 어플리케이션
    'django.contrib.sites', # 이것도 디스커스 사용하려면 세팅해줘야함
]
#disqus 사이트에서 발급받은 shortname 저장
DISQUS_WEBSITE_SHORTNAME = 'testdjangopage-1'
#웹 프로젝트별 개별적인 id값 설정
SITE_ID = 1 #식별자
MIDDLEWARE = [
...
STATIC_URL = '/static/'
#URL 주소로 파일 주소를 접근할 때 사용하는 경로
#http://127.0.0.1:8000/files/ 로 시작하는 경로는 미디어파일을 불러오는 것으로 판단함
MEDIA_URL = '/files/'
#os.path.join(기존경로,새경로) : 기존경로에 새경로를 붙인 문자열을 반환
#실제로 이미지/파일/동영상/한글/파워포인트 ,즉 모든파일이 저장되는 경로
MEDIA_ROOT = os.path.join(BASE_DIR,'files') #src/files 폴더안에 미디어 파일을 저장
