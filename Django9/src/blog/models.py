...
class Post(models.Model):
    ...
    #models.SET_NULL : 연결된 객체가 삭제되면 null 값을 가지는 역할
    #modles.SET_DEFAULT : 연결된 객체가 삭제되면 기본 객체와 연결되는 역할
    #models.SET(삭제되면 연결할 객체명) : 연결된 객체가 삭제되면 특정함수로 새로운 객체를 찾아 연결하는 역할
    #글 제목(headline) : CharField, 글자수 200자 제한
    headline = models.CharField('제목', max_length=200)
    #글 내용(content) : TextField(글자 수 제한 없는 텍스트 저장 공간)
    content = models.TextField('내용')
    #작성자(author) : ForeignKey, Django.auth.models.User 모델클래스와 연동
    #settings.AUTH_USER_MODEL : 현재 웹서버에 설정된 모델클래스를 의미(장고에서 제공해주는 User말고 다른 곳에서 가져온 유저모델 쓰고싶을 때)
    ...
    class Meta: #모델 클래스도 Meta 클래스를 정의해서 추가적인 정렬, 정보를 제공
        ...
        #기본적으로 오름차순으로 정렬함. 변수명 앞에 '-' 기호를 붙이면 내림차순으로 정렬
...
class PostImage(models.Model):
    ...
    # %m : 해당 서버의 월
    # %d : 해당 서버의 일
    image = models.ImageField('이지미파일',upload_to="images/%Y/%m/%d")
    #객체 삭제 시 image에 저장한 파일 경로의 파일 삭제 처리
    def delete(self, using=None, keep_parents=False):
        #image 변수에 저장된 경로의 파일을 지우는 작업
        # 왜? PostImage 객체가 지워질 때 image 변수가 가리키는 경로의 파일이 지워지지 않음
        #ImageField/FileField 사용시 객체가 삭제될 때 호출되는 delete 함수를 오버라이딩해서 
...
class PostFile(models.Model):
    ...
    #객체 삭제 시 file에 저장한 파일 경로에 해당하는 파일을 삭제하는 처리
    def delete(self, using=None, keep_parents=False):
        self.file.delete()
        return models.Model.delete(self, using=using, keep_parents=keep_parents)